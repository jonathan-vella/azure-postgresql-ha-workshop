...
var port_env = Environment.GetEnvironmentVariable("PORT") ?? "80";
Console.WriteLine($"Starting server on port {port_env}");
app.Run($"http://0.0.0.0:{port_env}");
                        ...
                        using (var conn = new NpgsqlConnection(connectionString))
                        {
                            ...
                            using (var cmd = conn.CreateCommand())
                            {
                                cmd.CommandText = @"
                                    ...
                                    VALUES (@customer_id, @merchant_id, @amount, 'completed') 
                                    RETURNING transaction_id";
                                
                                cmd.Parameters.AddWithValue("customer_id", Random.Shared.Next(1, 6));
                                cmd.Parameters.AddWithValue("merchant_id", Random.Shared.Next(1, 6));
